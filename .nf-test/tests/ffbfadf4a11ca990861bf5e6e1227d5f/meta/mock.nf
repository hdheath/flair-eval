...
include { SummaryPlots } from '/private/groups/brookslab/hdheath/projects/flair-eval/tests/modules/visualization/summary/../../../../modules/visualization/summary/main.nf'
...
workflow {
                ...
                input[0] = Channel.of(
                    tuple(
                        ...
                        [file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_evaluation.tsv"),
                         file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_evaluation_2.tsv")]
                    )
                )
    ...
    //run process
    SummaryPlots.run(input.toArray())

    if (SummaryPlots.output){

        // consumes all named output channels and stores items in a json file
        SummaryPlots.out.getNames().each { name ->
            serializeChannel(name, SummaryPlots.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = SummaryPlots.out as List<Object>
        def i = 0
        array.each { output ->
