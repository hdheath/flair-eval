...
include { PrepareReferencePeaks } from '/private/groups/brookslab/hdheath/projects/flair-eval/tests/modules/evaluation/prepare_ref_peaks/../../../../modules/evaluation/prepare_ref_peaks/main.nf'
...
workflow {
    ...
    //run process
    PrepareReferencePeaks.run(input.toArray())

    if (PrepareReferencePeaks.output){

        // consumes all named output channels and stores items in a json file
        PrepareReferencePeaks.out.getNames().each { name ->
            serializeChannel(name, PrepareReferencePeaks.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = PrepareReferencePeaks.out as List<Object>
        def i = 0
        array.each { output ->
