...
include { UTRFeatures } from '/private/groups/brookslab/hdheath/projects/flair-eval/tests/modules/divergence/utr_features/../../../../modules/divergence/utr_features/main.nf'
...
workflow {
    ...
    //run process
    UTRFeatures.run(input.toArray())

    if (UTRFeatures.output){

        // consumes all named output channels and stores items in a json file
        UTRFeatures.out.getNames().each { name ->
            serializeChannel(name, UTRFeatures.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = UTRFeatures.out as List<Object>
        def i = 0
        array.each { output ->
