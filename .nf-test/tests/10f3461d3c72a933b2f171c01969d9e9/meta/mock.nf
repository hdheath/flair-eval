...
include { PeakReasonHeatmap } from '/private/groups/brookslab/hdheath/projects/flair-eval/tests/modules/visualization/summary/../../../../modules/visualization/summary/main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = Channel.of(
                    tuple(
                        'test_set',
                        [file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_cage_peak_reasons.tsv"),
                         file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_quantseq_peak_reasons.tsv")]
                    )
                )
    ...
    //run process
    PeakReasonHeatmap.run(input.toArray())

    if (PeakReasonHeatmap.output){

        // consumes all named output channels and stores items in a json file
        PeakReasonHeatmap.out.getNames().each { name ->
            serializeChannel(name, PeakReasonHeatmap.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = PeakReasonHeatmap.out as List<Object>
        def i = 0
        array.each { output ->
