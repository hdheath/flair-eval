...
include { ToolDivergence } from '/private/groups/brookslab/hdheath/projects/flair-eval/tests/modules/divergence/tool_divergence/../../../../modules/divergence/tool_divergence/main.nf'
...
workflow {
                ...
                input[0] = Channel.of(
                    tuple(
                        ...
                        [file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_isoforms.bed"),
                         file("/private/groups/brookslab/hdheath/projects/flair-eval/tests/data/tiny_isoforms_tool2.bed")],
    ...
    //run process
    ToolDivergence.run(input.toArray())

    if (ToolDivergence.output){

        // consumes all named output channels and stores items in a json file
        ToolDivergence.out.getNames().each { name ->
            serializeChannel(name, ToolDivergence.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = ToolDivergence.out as List<Object>
        def i = 0
        array.each { output ->
