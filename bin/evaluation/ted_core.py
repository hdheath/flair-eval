...
def tss_tts_metrics(
    ...
    try:
        ...
        # CAGE/dRNA get read end positions for recoverability; ref peaks do not
        ...
        if test_regions_dir:
            ...
            if recov_3prime and peaks.get("prime3"):
                write_recoverable_peaks_bed(
                    ...
                    test_regions_dir / f"{plot_prefix}_recoverable_drna_peaks.bed")
        ...
        if plot_output_dir and HAS_MATPLOTLIB:
            ...
            # For dRNA (experimental 3')
            if max_count_drna is not None:
                final_max_drna = max_count_drna
            else:
                final_max_drna = calculate_histogram_max_count(dists_3prime) if dists_3prime else None
            ...
            # Plot dRNA (3' end) distance histogram -- colored by recoverability
            if dists_3prime:
                drna_plot_path = plot_output_dir / f"{plot_prefix}_drna_distance_histogram.png"
                if mask_3prime and len(mask_3prime) == len(dists_3prime):
                    plot_distance_histogram_colored(
                        distances=dists_3prime,
                        recoverable_mask=mask_3prime,
                        output_path=drna_plot_path,
                        title="3' End Distance to Nearest dRNA Peak",
                        max_count=final_max_drna,
                    )
                else:
                    plot_distance_histogram(
                        distances=dists_3prime,
            ...
            if recov_3prime and closest_3prime:
                ...
                if missed_3:
                    plot_read_support_distribution(
                        ...
                        output_path=plot_output_dir / f"{plot_prefix}_drna_peak_read_support.png",
                        title="Read Support at Missed Recoverable dRNA Peaks",
                    )
            ...
            if peaks.get("prime3") and closest_3prime:
                drna_meta = _build_peak_metadata(peaks["prime3"], recov_3prime, captured_3)
                if drna_meta:
                    total_3 = len(drna_meta)
                    captured_3_count = sum(1 for p in drna_meta if p['status'] == 'captured')
                    missed_3_count = sum(1 for p in drna_meta if p['status'] == 'missed')
                    no_reads_3 = sum(1 for p in drna_meta if p['status'] == 'no_reads')
                    ...
                    plot_peak_recovery_by_expression(
                        peak_metadata=drna_meta,
                        output_path=plot_output_dir / f"{plot_prefix}_drna_recovery_by_expression.png",
                        title="dRNA Peak Recovery by Expression Level",
                    )
            ...
            if map_file and map_file.exists() and reads_bed and reads_bed.exists():
                ...
                # Analyze missed dRNA peaks (3' ends)
                if missed_3 and peaks.get("prime3"):
                    ...
                    # Add SJ support summary metrics for dRNA
                    ...
                    if analysis_3.get('classification_summary'):
                        plot_read_classification_summary(
                            ...
                            output_path=plot_output_dir / f"{plot_prefix}_drna_read_classification.png",
                            title="Read Assignment at Missed dRNA Peaks",
                        )

                    # Plot SJ support for missed dRNA peaks
                    if analysis_3.get('sj_support_summary'):
                        plot_missed_peak_sj_support(
                            sj_support_summary=analysis_3['sj_support_summary'],
                            output_path=plot_output_dir / f"{plot_prefix}_drna_missed_sj_support.png",
                            title="SJ Support at Missed Recoverable dRNA Peaks",
                        )
                    ...
                    if test_regions_dir:
                        write_annotated_peaks_bed(
                            ...
                            output_path=test_regions_dir / f"{plot_prefix}_missed_drna_peaks_annotated.csv",
                            end_type='tts',
                        )

                        # Write troubled regions TSV with IGV coordinates (no truncation patterns for dRNA)
                        write_troubled_regions_tsv(
                            missed_peaks=missed_3,
                            peak_patterns={},  # No truncation analysis for dRNA
                            analysis_results=analysis_3,
                            output_path=test_regions_dir / f"{plot_prefix}_missed_drna_peaks.tsv",
                        )
...
def calculate_ted_metrics(
    ...
    window_5prime: Optional[int] = None,
    window_3prime: Optional[int] = None,
    cage_signal_plus: Optional[Path] = None,
    ...
    Args:
        ...
        prime3_peaks: Experimental 3' peaks (dRNA/PolyA)
