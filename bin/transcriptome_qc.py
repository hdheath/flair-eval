#!/usr/bin/env python3
"""Placeholder QC generation for FLAIR transcriptome runs."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate placeholder QC for FLAIR transcriptome runs.")
    parser.add_argument("--dataset", required=True, help="Dataset name for this transcriptome run.")
    ...
    parser.add_argument("--mode", required=True, help="Source mode for transcriptome (region or all).")
    parser.add_argument(
        "--region-tag",
        required=False,
        default="",
        help="Region tag used for naming transcriptome outputs.",
    )
    parser.add_argument("--stdout", required=True, type=Path, help="Transcriptome stdout file.")
    parser.add_argument("--stderr", required=True, type=Path, help="Transcriptome stderr file.")
    parser.add_argument("--metadata", required=True, type=Path, help="Transcriptome metadata JSON file.")
    parser.add_argument("--isoforms-bed", required=True, type=Path, help="Transcriptome isoforms BED output.")
    parser.add_argument("--isoforms-gtf", required=True, type=Path, help="Transcriptome isoforms GTF output.")
    parser.add_argument("--isoforms-fa", required=True, type=Path, help="Transcriptome isoforms FASTA output.")
    parser.add_argument("--read-map", required=True, type=Path, help="Transcriptome read map output.")
    ...
    read_map_stats = _summarise_file(args.read_map)
    ...
    rows: list[tuple[str, Any]] = [
        ...
        ("read_map_exists", "yes" if read_map_stats["exists"] else "no"),
        ("read_map_lines", read_map_stats["lines"]),
    ]
