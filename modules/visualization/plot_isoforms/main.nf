...
process PlotIsoforms {
    ...
    tuple val(test_name), val(dataset_name), val(align_mode), val(partition_mode), val(transcriptome_mode),
          path(bam), path(bai), val(region_spec), path(cage_peaks), path(quantseq_peaks),
    ...
    def quantseq_peaks_arg = quantseq_peaks.name != 'NO_QUANTSEQ' ? "--quantseq-peaks ${quantseq_peaks}" : ""
    ...
    def quantseq_signal_plus_arg = quantseq_signal_plus ? "--quantseq-signal-plus ${quantseq_signal_plus}" : ""
    def quantseq_signal_minus_arg = quantseq_signal_minus ? "--quantseq-signal-minus ${quantseq_signal_minus}" : ""
    ...
    python ${projectDir}/bin/isoform_plot_test.py \\
        ...
        ${quantseq_signal_plus_arg} \\
        ${quantseq_signal_minus_arg} \\
